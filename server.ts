...
} from './server/visionProviders.ts';
import {
  getSession,
  saveSession,
  addLog,
...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    // Vercel serves everything under public/** via its own CDN and ignores
    // express.static() entirely, so this block only actually runs on a
    // traditional persistent-Node host (Railway/Render/VPS) - harmless dead
    // code on Vercel. vite.config.ts builds the frontend to `public/` (not
    // `dist/`) specifically so both this fallback AND Vercel's own static
    // serving point at the same built assets.
